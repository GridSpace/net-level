...
const CLI = {
    ...
    cull: async function(gte, lt) {
        return CLI.clear({ gte, lt });
    },

    clear: async function(opts) {
    ...
    dump: async function (opts) {
        opts = evil(opts);
        let count = 0;
        let outFile = fs.openSync(opts.file || 'dump.base', opts.mode || 'w');
        return client.list(opts, (key, value) => {
...
function nextCmd(line) {
    ...
    let fn = CLI[cmd];
    if (fn) {
        return fn(...line).then(prompt).catch(error(cmd));
    }
    switch (cmd) {
        case 'exit':
        case 'quit':
