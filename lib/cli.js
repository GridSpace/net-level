...
let client = new (require('../index').client)();
...
function help(args) {
    if (args.length) {
        switch(args[0]) {
            case 'user': return help_user(args.slice(1));
        }
    }
    console.log([
...
function next(line) {
    line = line.toString().trim().split(' ').map(v => v.trim());
    let cmd = line.shift();
    switch (cmd) {
        case '?':
        case 'help':
        ...
        case 'open':
            client.close()
                ...
                .then(prompt).catch(error('open'));
            break;
        case 'user':
            client.user(line[0], line[1], evil(line[2]))
                .then(reply => {
                    if (reply.list || reply.rec) {
                        console.log(reply.list || reply.rec);
                    }
                })
                .then(prompt).catch(error('user'));
            break;
        case 'stat':
            let opts = {};
            if (line[0]) {
        ...
        case 'use':
            ...
            client.use(dbn, enc).then(() => {
                current = Array.isArray(dbn) ? '' : dbn;
            }).then(prompt).catch(error('use'));
            break;
        case 'get':
            client.get(line[0]).then(reply => {
                console.log(reply);
                prompt();
            }).catch(error('get'));
            break;
