...
function prompt(reply) {
    ...
    if (verbose) {
        cmdline.question(`:${client.path.join('/')}: `).then(next);
    } else {
...
function next(line) {
    ...
    switch (cmd) {
        ...
        case 'use': {
            ...
            client
                .use(dbn, enc)
                .then((rec) => (this.path = rec ? rec.path : []))
                .then(prompt)
                .catch(error('use'));
            break;
        }
        case 'sub': {
            client.sub(line).then(prompt).catch(error('sub'));
            break;
        }
        case 'clear': {
            client.clear(line[0]).then(prompt).catch(error('clear'));
            break;
        }
        case 'get':
            client
        ...
        case 'load':
            ...
            stream = fs.createReadStream(line[0]);
