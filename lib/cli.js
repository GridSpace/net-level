...
const CLI = {
    ...
    more() { return client.more() },

    async exit() { exiting = true },

    async quit() { return CLI.exit() }
};

function nextCmd(line) {
    const args = line.toString().trim().split(' ').map((v) => v.trim());
    const cmd = args.shift();
    const fn = CLI[cmd];
    if (fn) {
        return fn(...args).then(prompt).catch(error(cmd));
    }
    switch (cmd) {
        case '': {
            prompt();
