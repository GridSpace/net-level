...
class NetDB {
    ...
    #init(conn) {
        ...
        new LineBuffer(conn, (line) => {
            ...
            let { fn, err } = this.calls[call] || {
                err(m) {
                    console.error('no call for', call, m);
                },
                fn(m) {
                    console.log('no call for', call, m);
                }
            };
            if (json.error) {
                err?.(json.error);
    ...
    user(user, cmd, opt) {
        return new Promise((resolve, reject) => {
            this.#send({ user, cmd, opt }, (reply) => {
                if (reply.error) {
                    reject(reply.error);
                } else {
                    resolve(reply.rec || reply.list)
                }
            }, reject);
        });
    }
    ...
    get(key, opt = {}) {
        return new Promise((resolve, reject) => {
            this.#send(
                { get: key, opt: { valueEncoding: opt.valueEncoding } },
    ...
    put(key, value, opt = {}) {
        return new Promise((resolve, reject) => {
            this.#send(
                { put: key, value, opt: { valueEncoding: opt.valueEncoding } },
