...
function init(args = { _: [] }) {
    ...
    let admins = [];
    let uname = args.user || process.env.DB_USER || undefined;
    let upass = args.pass || process.env.DB_PASS || undefined;
    let upash = args.pash || process.env.DB_PASH || (upass ? hash(upass) : undefined);
    let port = args.port || parseInt(process.env.DB_PORT || 3000);
    let host = args.host || undefined;
    let ddir = args.dir || args.base || process.env.DB_BASE || 'data';
    let udbf = `${ddir}/.users`;
    let nuid = Date.now();
    let users = {};
    let base = {};
    ...
    function on_line(conn, state, line) {
        ...
        if (halt && perms.halt) {
        ...
        } else if (user && (perms.user || admins.length === 0)) {
            ...
            switch (user) {
                case 'add':
                    if (!users[cmd]) {
                        const perms = Object.assign(clone(defperms), opt);
            ...
            let range = rangeperm === true ? Infinity :
                Math.max(rangeperm, perms.range || 0, state.dbperms.range || 0);
            if (range === 0) {
                return conn.write(encode({ call, list, error: 'not authorized' }));
            }
            let read = 0;
