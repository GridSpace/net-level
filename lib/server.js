...
class LevelServer {
    ...
    #start_server() {
        if (!this.#server) {
            ...
            const server = this.#server = net
                .createServer(on_conn)
    ...
    #cmd_clear(conn, call, clear) {
        const { state } = conn;
        const { addr, db, check, perms } = state;
    ...
    #cmd_del(conn, call, del) {
        const { server, state } = conn;
        const { addr, check, db, dnam, drec, perms } = state;
        if (!(perms.del || state.dbperms.del)) {
            return conn.write(encode({ call, put, error: 'not authorized' }));
        }
        if (check(call))
