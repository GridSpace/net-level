...
function init(args) {
  ...
  let upass =
    process.env.DB_PASS ||
    args.pass ||
    Date.now().toString(36) + (Math.random() * 0xffffff).toString(36);
      ...
      new LineBuffer(conn, (line) => {
        ...
        if (halt && perms.halt) {
          ...
          switch (user) {
            ...
            case "del":
              if (users[cmd]) {
                ...
                return conn.write(
                  encode({ call, user, error: "no such user" })
                );
              }
              break;
            case "pass":
            ...
            case "list":
              if (cmd) {
                ...
                return conn.write(
                  encode({ call, user, list: Object.keys(users) })
                );
              }
          }
          conn.write(encode({ call, user }));
          ...
          if (
            !(
              perms.use ||
              (urec && urec.base && urec.base[use] && urec.base[use].use)
            )
          ) {
