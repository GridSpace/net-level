...
class LevelServer {
    ...
    #on_conn(conn) {
        ...
        function close(drec) {
            if (drec) {
                ...
                delete server.#uidname[state.uuid];
            }
        }

        function check_perms(bool) {
            if (!bool) {
    ...
    async #cmd_stat(conn, send, opt) {
        ...
        if (opt.list) {
            ...
            const dstat = await fs.stat(`${server.#ddir}/${opt.name}`);
            const btime = dstat.birthtimeMs;
            stat = server.#base[opt.name] || undefined;
            if (stat) {
                const { uids, gets, puts, dels, iter } = stat;
                const { options } = stat.db;
                const users = uids.map((uuid) => server.#uidname[uuid]);
                stat = { ssns: uids.length, gets, puts, dels, iter, options, users, btime };
            } else {
                stat = { btime };
            }
        }
    ...
    #cmd_use(conn, send, use, opt) {
        ...
        if (drec) {
            ...
            if (!(lastmod(path) || opt.create)) {
                return send({ error: 'no base, missing create parameter' });
            }
    ...
    #on_line(conn, state, line) {
        ...
        const { check_perms, check_db, perms } = state;
        ...
        if (halt && check_perms(perms.halt)) {
            this.#cmd_halt(conn, send, halt);
        } else if (stat) {
            this.#cmd_stat(conn, send, opt);
        } else if (user && check_perms(perms.user || this.#admins.length === 0)) {
            this.#cmd_user(conn, send, user, cmd, opt);
        } else if (drop && check_perms(perms.drop)) {
